...
class AISO:
    ...
    def __init__(self, num_cel, num_ger, num_par, num_rod, file, game_theory=True, evolutionary=True):
        ...
        self.__arquivo_memoria = self.__file.replace('.tsp', '') + self.__e * "_E" + self.__gt * "_GT" \
                                 + str(num_cel) + '.ggf'
    ...
    def executar(self):
        ...
        pop = Gerador(self.__num_cel, cidades, evolutionary=self.__e)
        ...
        for ger in range(self.__num_ger):
            ...
            num_clones = int(self.__num_cel*.3)
            ...
            fim = time.time()
            print(ger, populacao[0].get_fitness(), populacao[0].get_afinidade()*100, fim-inicio)
